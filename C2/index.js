...
const connect = ()=>{
    return mongoose.connect("mongodb+srv://vaishnavi:vaishnavi@cluster0.afine.mongodb.net/Bank_system?retryWrites=true&w=majority")
}
...
const Masteraccountschema = mongoose.Schema({
    ...
    branchDetailid :[{type : mongoose.Schema.Types.ObjectId,ref:"branchDetail",required:true}],
    mangerId : {type : mongoose.Schema.Types.ObjectId,ref:"user",required:true},
},{
    timestamps:true
})
...
const savingaccountschema = mongoose.Schema({
    ...
    UserId : {type : mongoose.Schema.Types.ObjectId,ref:"user",required:true},
    masterId : {type : mongoose.Schema.Types.ObjectId,ref:"masterbranch",required:true},
},{
    timestamps:true
})
...
app.get("/masteraccountinfo", async(req,res)=>{
    try {
       ...
       const Detail = await Masteraccount.find().populate({path : "UserId",select :["firstName","lastName","email","type"]}).populate({path : "mangerId",select:["type","firstName","lastName"]}).lean().exec()
...
app.get("/onlyaccandbalance", async(res,req)=>{
    try {
        const balanceandacc = await list.find().populate({path:"masterId",select:["balance"]}).lean().exec()
